#if !defined(AOC_Y2024_UTILS_HPP)
#define AOC_Y2024_UTILS_HPP (1)
...
template <typename T, char DELIMITER> AOC_Y2024_CONSTEXPR std::vector<T> parse_to_vector_of_numbers(std::string_view str)
{
    std::vector<T> ret{};
...
{
  public:
    [[nodiscard]] AOC_Y2024_CONSTEXPR Vector2D(std::size_t width, std::vector<T> &&vec) : m_width(width), m_vec(vec)
    {
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR T &operator()(std::size_t col, std::size_t row)
    {
        return m_vec[m_width * row + col];
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR const T &operator()(std::size_t col, std::size_t row) const
    {
        return m_vec[m_width * row + col];
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR T &operator[](std::size_t pos)
    {
        return m_vec[pos];
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR const T &operator[](std::size_t pos) const
    {
        return m_vec[pos];
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto width() const
    {
        return m_width;
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto height() const
    {
        const auto w = width();
        const auto s = size();
    ...
    [[nodiscard]] AOC_Y2024_CONSTEXPR auto size() const
    {
        return m_vec.size();
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto empty() const
    {
        return m_vec.empty();
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto begin() const
    {
        return m_vec.begin();
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto end() const
    {
        return m_vec.end();
    }

    [[nodiscard]] AOC_Y2024_CONSTEXPR auto square() const
    {
...
[[nodiscard]] AOC_Y2024_CONSTEXPR inline Vector2D<std::uint8_t> parse_to_vector_2d(std::string_view input,
...
[[nodiscard]] AOC_Y2024_CONSTEXPR auto abs(const auto &v)
{
    return (v < 0) ? (-v) : v;
}
...
#endif // !defined(AOC_Y2024_UTILS_HPP)
